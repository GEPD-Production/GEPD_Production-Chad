...
data_set_updated <- read_csv(paste(sample_folder, '/GEPD_TCD_weights_', currentDate,  '.csv', sep="")) %>%
  filter(
    !(school_code == 223748 & IDEN == "N DJAMENA 8"),
    !(school_code==585 & TELEPHONE_ETABLISSEMENT!="63170595"),
  ...
  select(school_code, Region, urban_rural, public, strata_prob, ipw)
...
school_dta<- school_dta %>%
  ...
  mutate(
    ...
    school_code=if_else(school_code==22760, 222760, school_code) #fix issue with school code
...
preamble_info <- c( 'interview__key', 'school_code',
                   ...
                   'school_code', 'Region', 'urban_rural', 'public', 'school_weight', 
...
teachers_dta<-read_dta(here(data_dir,"School", "TCD_teacher_level.dta")) %>%
  select(-urban_rural, -public, -rural) %>%
  mutate(teacher_name=m2saq2,
         teachers_id=if_else(is.na(teachers_id),m4saq1_number, teachers_id),
         teacher_number=teachers_id) %>%
  mutate(school_code=if_else(school_code==22760, 222760, school_code)) %>% #fix issue with school code
  left_join(school_collapse_temp %>% select(school_code, Region, urban_rural, public, school_weight, numEligible, numEligible4th )) %>%
...
teachers_dta<- teachers_dta %>%
  mutate(teacher_name=m3sb_troster  ,
         ...
         teacher_grd1=bin_var(m3saq2_1,1),
         teacher_grd2=bin_var(m3saq2_2,1),
         teacher_grd3=bin_var(m3saq2_3,1),
         teacher_grd4=bin_var(m3saq2_4,1),
         #teacher_grd5=bin_var(m3saq2_5,1),
         teacher_language=bin_var(m3saq3_1,1),
         teacher_math=bin_var(m3saq3_2,1),
         teacher_both_subj=bin_var(m3saq3_3,1),
         teacher_other_subj=bin_var(m3saq3_97,1),
...
teachers_dta <- teachers_dta %>%
  group_by(school_code) %>%
  mutate(
    g4_teacher_count=sum(m3saq2_4, na.rm=TRUE),
    g1_teacher_count=sum(m3saq2_1, na.rm=TRUE)
  )
...
#read in teach data
  teach_dta <- teachers_dta %>% 
    select(school_code, teachers_id, m2saq3, starts_with("s_"), starts_with("s2_")) %>% 
  ...
  pivot_longer(
    ...
    pivot_wider(id_cols =c(id, school_code, teachers_id, m2saq3, school_clip), names_from = behaviour, values_from = count) %>% select(-id) 
  ...
  teach_dta <- teach_dta %>% 
    group_by(school_code, teachers_id,m2saq3, school_clip) %>% 
...
#   select(school_code, everything())
  ...
  teacher_pedagogy_segments <- teach_dta %>%
    ...
    select(school_code, teachers_id, m2saq3, clip, everything()) 
   # mutate(across(everything(), as.numeric))
  ...
  teacher_pedagogy_segments <- teacher_pedagogy_segments %>%
    ...
    select(school_code, teachers_id, teach_score, classroom_culture, instruction, socio_emotional_skills, timeontask1, tt_low, tt_medium, tt_high, number_segments, teach_prof, classroom_culture_prof, instruction_prof, socio_emotional_skills_prof) 
...
teachers_dta <- teachers_dta %>%
  mutate(classroom_observed=bin_var(m3sdq15_ildr,1),
         ...
         purpose_observation=case_when(
           m3sdq18_ildr_1==1 ~ "Evaluation",
           m3sdq18_ildr_2==1 ~ "Professional Development",
           m3sdq18_ildr_3==1 ~ "Monitoring",
           m3sdq18_ildr_97==1 ~ m3sdq18_other_ildr ),
         ...
         feedback_observation=if_else((m3sdq21_ildr==1 & (m3sdq22_ildr_1==1 | m3sdq22_ildr_2==1 | m3sdq22_ildr_3==1
                                                          | m3sdq22_ildr_4==1 | m3sdq22_ildr_5==1)),1,0), #got feedback and was specific
...
teachers_dta <- teachers_dta %>%
  mutate(
    teacher_selection=case_when(
      (m3sdq1_tsdp_5==1 | m3sdq1_tsdp_6==1 | m3sdq1_tsdp_8==1 | m3sdq1_tsdp_9==1 )  ~ 2,
      (m3sdq1_tsdp_1==1 | m3sdq1_tsdp_2==1 | m3sdq1_tsdp_3==1 | m3sdq1_tsdp_4==1 | m3sdq1_tsdp_7==1) ~ 1,
      (m3sdq1_tsdp_1==0 & m3sdq1_tsdp_2==0 & m3sdq1_tsdp_3==0 & m3sdq1_tsdp_4==0 & m3sdq1_tsdp_5==0 & m3sdq1_tsdp_6==0 & m3sdq1_tsdp_7==0 & m3sdq1_tsdp_8==0 & m3sdq1_tsdp_9==0) ~ 0
    ),
    teacher_deployment=case_when(
      (m3seq8_tsdp_3==1 | m3seq8_tsdp_4==1 | m3seq8_tsdp_5==1  )  ~ 2,
      (m3seq8_tsdp_1==1 | m3seq8_tsdp_2==1 | m3seq8_tsdp_97==1) ~ 1,
      ((m3seq8_tsdp_1==0 & m3seq8_tsdp_2==0 & m3seq8_tsdp_3==0 & m3seq8_tsdp_4==0 & m3seq8_tsdp_5==0) | ( m3seq8_tsdp_99==1)) ~ 0
...
tevl<-c(
  'm3sbq7_tmna_1', 'm3sbq7_tmna_2', 'm3sbq7_tmna_3', 'm3sbq7_tmna_4','m3sbq7_tmna_5', 'm3sbq7_tmna_6', 'm3sbq7_tmna_97',
  'm3sbq8_tmna_2', 'm3sbq8_tmna_3', 'm3sbq8_tmna_4','m3sbq8_tmna_5', 'm3sbq8_tmna_6', 'm3sbq8_tmna_7', 'm3sbq8_tmna_8', 'm3sbq8_tmna_97', 'm3sbq8_tmna_98',
  'm3sbq9_tmna_1', 'm3sbq9_tmna_2', 'm3sbq9_tmna_3', 'm3sbq9_tmna_4', 'm3sbq9_tmna_7', 'm3sbq9_tmna_97', 'm3sbq9_tmna_98',
  'm3bq10_tmna_1', 'm3bq10_tmna_2', 'm3bq10_tmna_3', 'm3bq10_tmna_4', 'm3bq10_tmna_7', 'm3bq10_tmna_97', 'm3bq10_tmna_98')
...
teachers_dta<- teachers_dta %>%
  mutate(formally_evaluated=bin_var(m3sbq6_tmna,1),
         evaluation_content=if_else(m3sbq6_tmna==1,
                                    (m3sbq8_tmna_1+m3sbq8_tmna_2+ m3sbq8_tmna_3 + m3sbq8_tmna_5 + m3sbq8_tmna_6)/5,
                                    0),
         negative_consequences=case_when(
           (m3sbq9_tmna_1==1 | m3sbq9_tmna_2==1 | m3sbq9_tmna_3==1 | m3sbq9_tmna_4==1 | m3sbq9_tmna_97==1) ~ 1,
           (is.na(m3sbq9_tmna_1) & is.na(m3sbq9_tmna_2) & is.na(m3sbq9_tmna_3) & is.na(m3sbq9_tmna_4) & is.na(m3sbq9_tmna_97)) ~ as.numeric(NA),
           TRUE ~ 0),
         positive_consequences=case_when(
           (m3bq10_tmna_1==1 | m3bq10_tmna_2==1 | m3bq10_tmna_3==1 | m3bq10_tmna_4==1 | m3bq10_tmna_97==1) ~ 1,
           (is.na(m3bq10_tmna_1) & is.na(m3bq10_tmna_2) & is.na(m3bq10_tmna_3) & is.na(m3bq10_tmna_4) & is.na(m3bq10_tmna_97)) ~ as.numeric(NA),
           TRUE ~ 0)
  ) %>%
...
teachers_dta <- teachers_dta %>%
  mutate(attendance_evaluated=if_else(m3sbq6_tmna==1,
         ...
         attendence_sanctions=case_when(
           (m3sbq2_tmna_1==1 | m3sbq2_tmna_2==1 | m3sbq2_tmna_3==1 | m3sbq2_tmna_4==1 | m3sbq2_tmna_97==1) ~ 1,
           (is.na(m3sbq2_tmna_1) & is.na(m3sbq2_tmna_2) & is.na(m3sbq2_tmna_3) & is.na(m3sbq2_tmna_4) & is.na(m3sbq2_tmna_97)) ~ as.numeric(NA),
           TRUE ~ 0
         ),
         miss_class_admin=case_when(
           (m3sbq1_tatt_1==1 | m3sbq1_tatt_2==1 | m3sbq1_tatt_3==1 | m3sbq1_tatt_97==1) ~ 1,
           (m3sbq1_tatt_1==0 & m3sbq1_tatt_2==0 & m3sbq1_tatt_3==0 & m3sbq1_tatt_97==0) ~ 0,
...
teachers_dta <- teachers_dta %>%
  ...
  mutate(acceptable_absent=(m3scq1_tinm+ m3scq2_tinm + m3scq3_tinm)/3,
         ...
         motivation_teaching=case_when(
           is.na(m3scq15_tinm_3) ~ as.numeric(NA),
           m3scq15_tinm_3>=1 ~ 0,
           (m3scq15_tinm_3<1 & (m3scq15_tinm_1>=1 | m3scq15_tinm_2>=1 | m3scq15_tinm_4>=1 | m3scq15_tinm_5>=1)) ~ 1,
           TRUE ~ as.numeric(NA)
         )) %>%
